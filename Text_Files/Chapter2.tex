...
\]
$U_{CNOT}$ is a layer of $CNOT$ gates, and $U_{H}$ is a layer of Hadamard gates, acting on a computational state $\ket{s}$, and with global phase factor $e\in\mathbb{Z}_{4}$.\label{def:dch}
\end{defn}
...
\[
    f(x) = \sum_{i}a_{i}x_{i} + 2\sum_{c,t}x_{j}x_{k}\;\text{mod}\;4 = L(x) + 2Q(x)
\]
...
Finally, we need to encode the action of $U_{D}$. The phase resulting from a single qubit diagonal Clifford is conditional on the qubits being in the $\ket{1}$ state. Thus, we can write the linear part of the weighted polynomial as $Lx^{T}$ for some row-vector $L$ of integers $\text{mod}\;4$, which we call the linear phase vector. Each value in $L$ can be stored using just 2 bits.\par 
...
\end{equation}
where $B$ is a matrix such that $B_{ii}=L_{i},\;B_{i,j}=Q_{i,j}$, as by definition $Q$ has zero diagonal. We refer to $B$ as simply the phase matrix, with diagonal elements stored $\text{mod}\,4$ and off-diagonal elements stored $\text{mod}\,2$.\par
Including with the global phase factor, an integer modulo $4$ which can stored using just two bits, the DCH representation is specified by the tuple $\left(e, s, h, B, W\right)$. The spatial complexity is thus $\Theta(n^{2})$. In order to optimize certain subroutines, which we discuss later in this section, we also store a copy of $W^{-1}$, the inverse of the CNOT matrix, and $W^{T}$, the transpose of the CNOT matrix. Overall, then, the DCH form requires roughly $4n^{2}+4n+2$ bits of memory.
\begin{defn}
CH Representation:\\
Any stabilizer state $\ket{\phi}$ can be written as
\begin{equation}
...
\end{equation}
$U_{H}$ is a layer of $H$ gates, $\ket{s}$ is a computational basis state and $e\in\mathbb{Z}_{4}$ is a global phase factor.\label{def:ch}
\end{defn}
%%Outline classical encoding here
...
Finally, we again require two further bits to encode the global phase, and the $CH$ representation is thus given by the tuple $(e, s, h, G, M, F)$. Overall, the $CH$ form  also has spatial complexity $\theta(n^{2})$.  In order to optimize some subroutines, we additionally store copies of $M^{T}$ and $F^{T}$, requiring a total of $5n^{2}+4n+2$ bits of memory.
...
\begin{align}
S_{i}\ket{\phi} &\implies B_{i,i}\leftarrow B_{i,i} + 1\;\text{mod}\,4 \\
Z_{i}\ket{\phi} = S^{2}\ket{\phi} &\implies B_{i,i}\leftarrow B_{i,i} +2\;\text{mod}\,4 \\
S_{i}^{\dagger} = s^{3}\ket{\phi} & \implies B_{i,i}\leftarrow B_{i,i} + 3\;\text{mod}\,4. 
\end{align}
...
&= CNOT_{c,t} U_{D} \ket{x + x_{j}e_{k}\;\text{mod}\,2} \nonumber \\
&= \mathi^{f(x + x_{j}e_{k})} CNOT_{c,t} \ket{x + x_{j}e_{k}\;\text{mod}\,2} \nonumber \\
&= \mathi^{f(x + x_{j}e_{k})} \ket{x + 2x_{j}e_{k}\;\text{mod}\,2} \nonumber \\
...
&= \mathi^{(xG)B(xG)^{T}}U_{C}^{\dagger}\ket{xG} \nonumber \\
&= \mathi^{(xG)B(xG)^{T}}\ket{xGG^{-1}} \nonumber \\
&= \mathi^{xGBG^{T}x^{t}}\ket{x},
\end{align}
completing the proof.
\end{proof}
In general, computing the updated form of $U_{CNOT}U_{D}U_{CNOT}^{\dagger}$ would require time $O(n^{2})$. However, for the case of a single gate $CNOT_{c,t}$, recall that the matrix $E$ differs from the identity matrix at a single element, $E_{c,t}=1$. This allows us to simplify the updates as
\begin{equation}
...
\[
\begin{array}{rcl}
\left[WF\right]_{ij} = &  \sum_{k}W_{i,k}F_{k,j} = & \begin{cases} G_{i,j} & j\neq t \\ G_{i,c}+G_{i,t} & j=t \end{cases}\\
\\
\left[FW^{-1}\right]_{i,j} = &  \sum_{k}F_{i,k}W_{k,j}^{-1} = & \begin{cases} W^{-1}_{i,k} & i\neq c \\ W^{-1}_{c,j}+GW^{-1}_{t,j} & i=c \end{cases}
\end{array}\]
...
\end{equation}
Because $C_{m+1}$ is a Clifford operator, the term $C_{m+1}PC_{m+1}$ is also a Pauli operator $i^{\alpha}\prod_{i=1}^{n}X_{i}^{x_{i}}Z_{i}^{z_{i}}$ for some phase $\alpha$ and bit strings $x$ and $z$. This allows us to write
\begin{align}
...
\gamma_{q} & \gets & \gamma_{q} + 3\;\text{mod}\,4 \\
\end{array}\right.
\]
We can compute the updates for $S^{\dagger}$, $CZ$ and $CX$ in the same way, giving overall gate update rules
\begin{align}
S_{q} & \left\{ \begin{array}{rcl}
\text{row}_{q}(M)  & \gets & \text{row}_{q}(M)+\text{row}_{q}(G) \\
\gamma_{q} & \gets & \gamma_{q} + 3\;\text{mod}\,4 \\
\end{array} \right. \nonumber \\
S^{\dagger}_{q} &\left\{
\begin{array}{rcl}
\text{row}_{q}(M)  & \gets & \text{row}_{q}(M)+\text{row}_{q}(G) \\
\gamma_{q} & \gets & \gamma_{q} + 1\;\text{mod}\,4
\end{array}\right. \nonumber \\
CZ_{q,p} & \left\{
\begin{array}{rcl}
...
\gamma_{q} & \gets & \gamma_{q}+\gamma_{p} + 2 \sum_{i}M_{q,i}F_{p,i} \;\text{mod}\,4
\end{array}\right.
\end{align}
On the final line, the term $MF^{T}$ applies an extra phase correction that results from reordering the Pauli operators in the CNOT updates. Expanding out the action on the $X$ stabilizers gives
\begin{align*}
...
\end{align*}
and we pick up an extra phase of $-1$ each time $M_{q,i}=F_{p,i}=1$.
