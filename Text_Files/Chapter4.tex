    ...
    \For{$\alpha_{i},\;\ket{\phi_{i}}\in\ket{\tilde{\phi}}$}
        ...
        \If{$\Gamma \neq 0$}
            \For{$\ket{\eta_{i}}\in \{\ket{\eta_{i}}\}$}
                \State{$\eta_{i}\gets \Gamma\,\alpha_{i}\,\braket{\eta_{i}}{\phi_{i}} + \eta_{i}$}
            \EndFor
        \EndIf
    \EndFor
...
\State{$p_{x}\gets \left|\sum_{i}\alpha_{i}\braket{\va{x}}{\phi_{i}}\right|^{2}$}
...
\end{figure}
\subsubsection{Random Circuit Models}\label{sec:google_circ_intro}
...
\input{Figures/RandomCircuits/connectivity_runtimes}
\end{scaletikzpicturetowidth}
